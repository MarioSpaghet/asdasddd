...
function CameraSystem.CalculateBasicRoll(weapon, ply, context)
    ...
    -- Wall run feature removed
    if cv.terrainType == "sloped" or cv.terrainType == "steep" then
...
function CameraSystem.CalculateView(weapon, ply, pos, angles, fov)
    ...
    if GetConVar("webswing_firstperson_mode"):GetBool() then
        -- First-person mode - position camera in ragdoll's head
        if weapon.RagdollActive and weapon.Ragdoll and IsValid(weapon.Ragdoll) then
            -- Find the head bone
            local headBone = weapon.Ragdoll:LookupBone("ValveBiped.Bip01_Head1")
            if headBone then
                local headPos, headAng = weapon.Ragdoll:GetBonePosition(headBone)
                if headPos then
                    -- Position camera slightly forward from the head bone
                    local view = {}
                    view.origin = headPos + headAng:Forward() * 4 -- Slightly forward from head center
                    view.angles = angles -- Use player's view angles
                    view.fov = fov
                    view.drawviewer = false -- Don't draw the player model
                    return view
                end
            end

            -- Fallback if head bone not found - use the ragdoll's position
            local ragdollPos = weapon.Ragdoll:GetPos()
            local view = {}
            view.origin = ragdollPos + Vector(0, 0, 64) -- Approximate head height
            view.angles = angles
            view.fov = fov
        ...
        -- If no ragdoll, use default first-person view
        return
    end
